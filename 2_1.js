...
for (const game of input) {
  let gameId = parseInt(game.split(':')[0].split(' ')[1]);

  let gameData = game.split(':')[1].trim().split(';');

  // console.log('gameData', gameData);

  gameDataLoop: for (const [i, round] of gameData.entries()) {
    ...
    for (const color of roundData) {
      ...
      if (parseInt(colorNumber) > limit[colorName]) {
